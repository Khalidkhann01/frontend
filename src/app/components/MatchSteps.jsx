...
export function ReadyStep({ match, email, onStart, loading }) {
        ...
        <div className="space-y-2 mb-4">
          <div className="space-y-1.5">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-white">{match.hardSkillsMatch}%</span>
            </div>
            <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-purple-500 to-purple-400 rounded-full transition-all duration-1000" style={{ width: `${match.hardSkillsMatch}%` }} />
            </div>
          </div>
          <div className="space-y-1.5">
            <div className="flex justify-between text-sm">
              <span className="text-gray-400">Soft skills</span>
              <span className="font-medium text-white">{match.softSkillsMatch}%</span>
            </div>
            <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-purple-500 to-purple-400 rounded-full transition-all duration-1000" style={{ width: `${match.softSkillsMatch}%` }} />
            </div>
          </div>
          <div className="space-y-1.5">
            <div className="flex justify-between text-sm">
              <span className="text-gray-400">Experience</span>
              <span className="font-medium text-white">{match.experienceMatch}%</span>
            </div>
            <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-purple-500 to-purple-400 rounded-full transition-all duration-1000" style={{ width: `${match.experienceMatch}%` }} />
            </div>
          </div>
          <div className="space-y-1.5">
            <div className="flex justify-between text-sm">
              <span className="text-gray-400">Education</span>
              <span className="font-medium text-white">{match.educationMatch}%</span>
            </div>
            <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-purple-500 to-purple-400 rounded-full transition-all duration-1000" style={{ width: `${match.educationMatch}%` }} />
            </div>
          </div>
        </div>
    ...
    <div className="max-w-xl mx-auto space-y-6 animate-slide-up">
      <div className="relative p-6 rounded-2xl bg-white/[0.03] backdrop-blur-sm border border-white/5">
        <div className="text-center mb-4">
          ...
          <p className="text-sm text-gray-400 mt-1">{data.mismatchSummary}</p>
        </div>

        <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 p-3 mb-4 text-center">
          <p className="text-xs font-semibold text-amber-400">Match score: {data.matchScore}%</p>
        </div>
        ...
        <div className="space-y-3 mb-5">
          {data.suggestedRoles && data.suggestedRoles.map((role, i) => (
